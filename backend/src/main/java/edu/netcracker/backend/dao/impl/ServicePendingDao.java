...
public class ServicePendingDao implements IPendingDao<ServicePendingActivationDto> {
    ...
    public List<ServicePendingActivationDto> getPendingEntries() {
        ...
        return jdbcTemplate.queryForList(PendingActivationScripts.GET_SERVICES_PENDING_ACTIVATION.script)
                .stream()
    ...
    public List<ServicePendingActivationDto> getPendingWithOffsetAndLimit(Number limit, Number offset) {
        ...
        return jdbcTemplate.queryForList(PendingActivationScripts.GET_SERVICES_PENDING_ACTIVATION.script + PendingActivationScripts.LIMIT_AND_OFFSET.script, new Object[]{limit, offset})
                .stream()
                .map(this::getPendingActivationService)
                .collect(Collectors.toList());
    }
